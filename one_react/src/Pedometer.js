...
const Pedometer = ({ userId }) => {
    ...
    useEffect(() => {
        if (profile.weight !== null) {
            ...
            setWeightInput('50'); // Default to 50kg if no weight is set
        }
    }, [profile.weight]);

    // --- Update Calorie Goal based on weight ---
    useEffect(() => {
        const newGoal = profile.weight ? Math.round(parseFloat(profile.weight) * 25) : Math.round(50 * 25); // Use 50kg as default for goal calculation
                                ...
                                <div className="weight-display-container">
                                    ...
                                    <div className="weight-value-container">
                                        <input
                                            ...
                                            {...(isEditingWeight && { autoFocus: true })}
                                        />
