...
app.use(
  cors({
    origin: ["http://localhost:3000", "https://oneday-b9a73.web.app", "https://oneday7-1f3ee.web.app", "https://54.180.83.24.nip.io"],
    credentials: true,
  })
);
...
app.use((req, res, next) => {
  if (!req.isAuthenticated() && !req.session.guestUserId) {
    req.session.guestUserId = 19; // Assign guest user ID
  }
  next();
});

app.use((req, res, next) => {
  if (!req.isAuthenticated() && req.session.guestUserId === 19) {
    // Manually "authenticate" the guest user
    req.user = { id: 19, username: 'Guest', email: 'guest@oneday.com', provider: 'guest' }; // Mock user object
  }
  next();
});
